...
`HumanA` holds a `Weapon &` (must be given one at construction, and can
never be weaponless); `HumanB` holds a `Weapon *` (may start unarmed, and
...
that choosing between a reference and a pointer is a design decision.
